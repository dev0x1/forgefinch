...
description: Use for SQLx schema checks, query validation, transaction boundaries, and local schema files inside a workpackage or slice.
---
...
Support SQLx schema checks, query validation, transaction boundaries, and local schema files inside the active workpackage.

## Use This Skill When

- The current workpackage or slice touches SQLx schema checks, query validation, transaction boundaries, and local schema files.
- A local check, review, or design choice needs this specialty.

## Workflow

1. Read `AGENTS.md`, `docs/process/WORKPACKAGE_SYSTEM.md`,
   `PROJECT_PHASE.toml`, the active workpackage YAML file and selected slice entry
   when present, and nearby code or docs.
...
- Read `PROJECT_PHASE.toml` before changing schema files, creating migration
  files, or adding migration-file tests.

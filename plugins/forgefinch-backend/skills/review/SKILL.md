---
name: review
description: Use for software review for boundaries, missing tests, security gaps, and tenant leakage inside a workpackage or slice.
---
...
Support software review for boundaries, missing tests, security gaps, and tenant leakage inside the active workpackage.

## Use This Skill When

- The current workpackage or slice touches software review for boundaries, missing tests, security gaps, and tenant leakage.
...
5. For a quality-review slice, independently inspect the complete implementation
   delta and report actionable findings before summaries. For final verification,
   confirm implementation and quality review are resolved and the complete
   goal/constraint matrix passes. A defect requires `[defect-open]`, reopening
   its owning implementation slice, and returning review and verification to
   `todo`.
6. Run targeted local checks when practical and record command names, results, changed files, and open questions.
